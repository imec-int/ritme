...
public class KgssServiceImpl implements KgssService {
    ...
    private static class KgssServiceHolder {
        private static final KgssService KGSS_SERVICE = new KgssServiceImpl();
    }
    ...
    public static KgssService getInstance() {
        return KgssServiceHolder.KGSS_SERVICE;
    }
    ...
    /* (non-Javadoc)
	 * @see be.technicalconnector.services.KgssService#retrieveNewKey(byte[], java.util.List, java.lang.Long, java.lang.Long, java.lang.String, byte[])
     */
    @Override
    public KeyResult retrieveNewKey(byte[] etkKgss, List<String> credentialTypes, Long prescriberId, Long executorId, String patientId, byte[] myEtk, final String nihiiOrg) throws IntegrationModuleException {
        ...
        for (String credentialTypeStr : credentialTypes) {
            ...
            if (atrs.length == 3) {
                value = atrs[2];
                value = value.replaceAll("%PRESCRIBER_ID%", ETKHelper.longToString(prescriberId, 11));
                value = value.replaceAll("%EXECUTOR_ID%", ETKHelper.longToString(executorId, 11));
        ...
        try {
            ...
            SessionItem sessionItem = PluginManager.get("ritme.outbound.hospital.sessionmanager", uz.ehealth.ritme.outbound.hospital.SessionService.class).getSessionManager(nihiiOrg).getSession();
