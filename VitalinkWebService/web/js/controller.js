...
var controller = function (controller, jQuery) {
    ...
    function changeDataset(dataset) {
        ...
        //var version = window.controller.versionDefault;
        //var autoAnimate = window.controller.autoAnimateDefault;

        // Note: controller assumes model has been initialized already
        window.controller.init(false, version, autoAnimate);
    ...
    function prepareHandlers() {
        ...
        $(".review").click(function () {
            ...
            var data = $.map(model.accepted, function (a) {
                return model.items[a].attributes[model.ATTR_FULLDATA];
            });

            feedback.reportAccepted(model.accepted, data);
        ...
        $(window).unbind("keydown")// for some reason, multiple keydowns firing, remove previous ones
            .keydown(function (event) {
                ...
                if (!$("input[name='filterOn']").is(':focus') && !$("input[name='name']").is(':focus') && !$("input[name='dose']").is(':focus') && !$("input[name='route']").is(':focus') && !$("input[name='frequency']").is(':focus') && !$("textarea[name='instructions']").is(':focus')) {
                    switch (event.which) {
                        case 48:
                            // the '0' key
                            setAnimationSpeed(ANIMATION_SPEED_0);
    ...
    function populateItems($container) {
        ...
        for (var id in model.items) {
            ...
            if (item.attributes[model.ATTR_FULLDATA]) {
                var object = item.attributes[model.ATTR_FULLDATA];
                if (object.patientOrigin) {
                    $item.addClass("patientOrigin");
                }
                if (!object.validated) {
                    $item.addClass("unvalidated");
                }
            }
    ...
    function mouseoverHandler() {
        ...
        for (var attributeName in model.attributes) {
            if (attributeName in item.attributes && model.attributes[attributeName].displayFullDescription) {
    ...
    function updateItem(id, allNames, attributes) {
        ...
        for (var aN in attributes) {
            var oldValue;
            var newValue;
            if (aN == model.ATTR_FULLDATA) {
                oldValue = item.attributes[aN];
                newValue = attributes[aN];
            } else {
                oldValue = item.attributes[aN] == undefined ? "" : item.attributes[aN].toString();
                newValue = attributes[aN] == undefined ? "" : attributes[aN].toString();
            }
    ...
    function saveModifiedItem(type) {
        ...
        var ids = $(".added input[name]").map(function (a, b) {
            return b.getAttribute("name")
        });
        var values = $(".added input[name]").map(function (a, b) {
            return b.value;
        });
        var patt = new RegExp("\\d+");
        var result = $.makeArray(ids).reduce(function (a, b, i) {
            var elements = b.split("-");
            elements.reduce(function (c, d, i) {
                if (!c[d]) {
                    if (i + 1 < elements.length && patt.test(elements[i + 1])) {
                        c[d] = [];
                    }
                    else {
                        c[d] = {};
        ...
        attributes[model.ATTR_ROUTE] = result.drugRoute ? [result.drugRoute] : [route];
        attributes[model.ATTR_FREQUENCY] = result.periodicity ? [result.periodicity] : [frequency];
        attributes[model.ATTR_INSTRUCTIONS] = result.instructionsForPatient ? [result.instructionsForPatient] : [instructions];
        attributes[model.ATTR_FULLDATA] = result;


        allNames = {
            recorded: result.intendedMedication.medicationDescription ? result.intendedMedication.medicationDescription : name,
            generic: result.intendedMedication.medicationDescription ? result.intendedMedication.medicationDescription : name,
            brand: result.intendedMedication.medicationDescription ? result.intendedMedication.medicationDescription : name
        };
    ...
    function resetModifyPanel(type) {
        ...
        if (type === "edit") {
            ...
            $(".fields > .added").remove();
            $("<div class='added'>").insertBefore($(".fields > a"));
            var insertion = $(".added");


            for (var field  in item.attributes[model.ATTR_FULLDATA]) {
                if (item.attributes[model.ATTR_FULLDATA].hasOwnProperty(field)) {
                    var index = field;
                    if (model.ITEM_FORMATTER[index]) {
                        insertion.append(model.ITEM_FORMATTER[index].toHTML(field, item.attributes[model.ATTR_FULLDATA][field]));
                        if (model.ITEM_FORMATTER[index]["postAdd"]) {
                            model.ITEM_FORMATTER[index].postAdd(field, item.attributes[model.ATTR_FULLDATA][field]);
                        }
                    }
                    else {
                        insertion.append("<label for='" + field + "'>" + field + "</label><input id='" + field + "' name='" + field + "' value='" + item.attributes[model.ATTR_FULLDATA][field] + "'>");
                    }
                }
            }
        }
    ...
    visible.changeDataset = changeDataset
