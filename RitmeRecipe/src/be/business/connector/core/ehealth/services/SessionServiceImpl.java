...
public class SessionServiceImpl implements SessionService {
	...
	protected static final String SESSIONMANAGER_SAMLATTRIBUTE = "sessionmanager.samlattribute";

	protected static final String SESSIONMANAGER_SAMLATTRIBUTEDESIGNATOR = "sessionmanager.samlattributedesignator";

	protected static final String SESSIONMANAGER_SAMLATTRIBUTE_MANDATE = "sessionmanager.samlattribute.mandate";

	protected static final String SESSIONMANAGER_SAMLATTRIBUTEDESIGNATOR_MANDATE = "sessionmanager.samlattributedesignator.mandate";
	...
	public String createFallbackSession(String identificationKeystorePassword, String holderOfKeyKeystorePassword, String encryptionKeystorePassword, final String nihiiOrg) throws IntegrationModuleException {
		...
		try {
			...
			if (e.getCause() != null && e.getCause() instanceof TechnicalConnectorException && StringUtils.equals(EHEALTH_ERROR_CODE_WRONG_KEYSTORE_PASSWORD, ((TechnicalConnectorException) e.getCause()).getErrorCode())) {
				String keyStorePath = StringUtils.replace(StringUtils.substringBetween(e.getCause().getMessage(), "(", ")"), "path=", "");
				throw new IntegrationModuleException(I18nHelper.getLabel("technical.connector.error.wrong.keystore.password", new Object[] { keyStorePath }), e);
			}
			throw new IntegrationModuleException(I18nHelper.getLabel("technical.connector.error.create.fallback.session"), e);
		} catch (SOAPFaultException e) {
	...
	protected void reloadSamlAttributesAndDesignators(String samlAttribute, String samlAttributeDesginator) {
		...
		for (int i = 0; i < samlAttributesInPropertyFile.size(); i++) {
			ConfigFactory.getConfigValidator(new ArrayList<String>()).setProperty(SESSIONMANAGER_SAMLATTRIBUTE + "." + (i + 1), samlAttributesInPropertyFile.get(i));
		}

		List<String> samlAttributesAfterRefill = ConfigFactory.getConfigValidator(new ArrayList<String>()).getMatchingProperties(SESSIONMANAGER_SAMLATTRIBUTE);
		...
		for (int i = 0; i < samlAttributeDesignatorsInPropertyFile.size(); i++) {
			ConfigFactory.getConfigValidator(new ArrayList<String>()).setProperty(SESSIONMANAGER_SAMLATTRIBUTEDESIGNATOR + "." + (i + 1), samlAttributeDesignatorsInPropertyFile.get(i));
		}

		List<String> samlAttributeDesignatorsAfterRefill = ConfigFactory.getConfigValidator(new ArrayList<String>()).getMatchingProperties(SESSIONMANAGER_SAMLATTRIBUTEDESIGNATOR);
