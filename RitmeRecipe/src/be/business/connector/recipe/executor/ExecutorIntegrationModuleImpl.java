...
public class ExecutorIntegrationModuleImpl extends AbstractIntegrationModule implements ExecutorIntegrationModule {
	...
	public ExecutorIntegrationModuleImpl(RequestorProvider requestorIntegrationModule, CommonIntegrationModule commonIntegrationModule, final String nihiiOrg) throws IntegrationModuleException {
		super();
	...
	public GetPrescriptionForExecutorResult getPrescription(String rid, final String nihiiOrg) throws IntegrationModuleException {
		...
		try {
			...
			be.fgov.ehealth.recipe.protocol.v2.GetPrescriptionForExecutorRequest request = createGetPrescriptionRequest(rid, marshaller, nihiiOrg);
			be.fgov.ehealth.recipe.protocol.v2.GetPrescriptionForExecutorResponse response = null;

			try {
				response = RecipeExecutorServiceImpl.getInstance().getPrescriptionForExecutor(request, nihiiOrg);
	...
	public void markAsArchived(String rid, final String nihiiOrg) throws IntegrationModuleException {
		...
		try {
			...
			// call WS
			try {
				checkStatus(RecipeExecutorServiceImpl.getInstance().markAsArchived(request, nihiiOrg));
			} catch (ClientTransportException cte) {
				throw new IntegrationModuleException(I18nHelper.getLabel("error.connection.executor"), cte);
			}
		} catch (Throwable t) {
	...
	public void markAsDelivered(String rid, final String nihiiOrg) throws IntegrationModuleException {
		...
		try {
			...
			try {
				checkStatus(RecipeExecutorServiceImpl.getInstance().markAsDelivered(request, nihiiOrg));
			} catch (ClientTransportException cte) {
				throw new IntegrationModuleException(I18nHelper.getLabel("error.connection.executor"), cte);
			}
		} catch (Throwable t) {
	...
	public void markAsUndelivered(String rid, final String nihiiOrg) throws IntegrationModuleException {
		...
		try {
			...
			try {
				checkStatus(RecipeExecutorServiceImpl.getInstance().markAsUnDelivered(request, nihiiOrg));
			} catch (ClientTransportException cte) {
				throw new IntegrationModuleException(I18nHelper.getLabel("error.connection.executor"), cte);
			}
		} catch (Throwable t) {
	...
	public void revokePrescription(String rid, String reason, final String nihiiOrg) throws IntegrationModuleException {
		...
		try {
			...
			try {
				checkStatus(RecipeExecutorServiceImpl.getInstance().revokePrescriptionForExecutor(request, nihiiOrg));
			} catch (ClientTransportException cte) {
				throw new IntegrationModuleException(I18nHelper.getLabel("error.connection.executor"), cte);
			}
	...
	public List<ListNotificationsItem> listNotifications(boolean readFlag, final String nihiiOrg) throws IntegrationModuleException {
		...
		try {
			...
			try {
				response = RecipeExecutorServiceImpl.getInstance().listNotifications(request, nihiiOrg);
			} catch (ClientTransportException cte) {
				throw new IntegrationModuleException(I18nHelper.getLabel("error.connection.executor"), cte);
			}
			checkStatus(response);
	...
	public void createFeedback(long prescriberId, String rid, byte[] feedbackText, final String nihiiOrg) throws IntegrationModuleException {
		...
		try {
			...
			for (int i = 0; i < etkRecipients.size(); i++) {
				...
				try {
					checkStatus(RecipeExecutorServiceImpl.getInstance().createFeedback(request, nihiiOrg));
				} catch (ClientTransportException cte) {
					throw new IntegrationModuleException(I18nHelper.getLabel("error.connection.executor"), cte);
				}
			}
	...
	private be.fgov.ehealth.recipe.core.v1.SecuredContentType createSecuredContentTypeV2(byte[] content) {
		be.fgov.ehealth.recipe.core.v1.SecuredContentType secured = new be.fgov.ehealth.recipe.core.v1.SecuredContentType();
		secured.setSecuredContent(content);
		return secured;
	}

	private void checkStatus(be.fgov.ehealth.commons.protocol.v1.ResponseType response) throws IntegrationModuleException {
	...
	private String getLocalisedMsg(StatusType status) {
		...
		for (be.fgov.ehealth.commons.core.v1.LocalisedString msg : status.getMessages()) {
	...
	private be.fgov.ehealth.commons.core.v1.IdentifierType createIdentifierType(final String id, final String type) {
