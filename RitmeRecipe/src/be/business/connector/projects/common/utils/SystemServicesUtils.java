...
import be.apb.gfddpp.systemservices.v2.SystemServices.SystemServicesList.SystemServicesEntry;
import be.apb.gfddpp.systemservices.v2.SystemServices.SystemServicesList.SystemServicesEntry.ContractList;
import be.apb.gfddpp.systemservices.v2.SystemServices.SystemServicesList.SystemServicesEntry.ContractList.ContractEntry;
import be.apb.gfddpp.systemservices.v2.SystemServices.SystemServicesList.SystemServicesEntry.ContractList.ContractEntry.ContractPartyList.ContractPartyEntry;
...
public class SystemServicesUtils {
	...
	public SystemServicesUtils(PropertyHandler propertyHandler) throws IntegrationModuleException {
		...
		systemServicesXmlFile = ConfigUtils.getLatestSystemServicesFile(systemServicesPath, propertyHandler);
		reloadCache();
	}
		...
		for (SystemServicesEntry systemServicesEntry : systemServicesCache.getSystemServicesList().getSystemServicesEntry()) {
			if (systemServicesEntry.getSystem().getSystemType().equals(systemType) && systemServicesEntry.getSystem().getSystemId().equals(tipId)) {
				for (int i = 0; i < systemServicesEntry.getService().size(); i++) {
					SystemServices.SystemServicesList.SystemServicesEntry.Service service = systemServicesEntry.getService().get(i);
